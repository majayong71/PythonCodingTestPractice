...
# n = 전체 학생 수, lost = 도난 당한 학생 번호, reverse = 여벌 체육 복을 가진 학생 번호.
def solution(n, lost, reverse):

    # 실제로 체육 복을 잃어 버린 학생들 (여벌이 있던 학생 제외)
    lost_set = set(lost) - set(reverse)

    # 실제로 빌려줄 수 있는 학생들 (자신도 잃어 버리지 않은)
    reverse_set = set(reverse) - set(lost)

    # 앞번호부터 차례로 빌려주기.
    for student in sorted(reverse_set):
...
print(solution(10, [2, 4, 6], [1, 3, 5, 7, 8]))  # Output: 10
print(solution(6, [1, 2, 3], [2, 3, 4]))  # Output: 5
print(solution(7, [1, 2, 3, 4], [5, 6]))  # Output: 4
